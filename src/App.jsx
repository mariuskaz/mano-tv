...
export default function App() {
    ...
    const Preview = () => {
      return (
        <div className="preview">
          <button 
            className={selectedChannelUrl.includes("lrt") ? "close-button left" : "close-button right"} 
            onClick={() => setSelectedChannelUrl(null)} /> 
          <iframe 
    ...
    return (
      <>
        ...
        {items === -1 && <Loader />}
        {items === 0 && 
          <Toast message={"Error fetching EPG data!"} button={"Retry"} action={() => setSynced(false)} />}
        {items > 0 && items < channels.length && 
          <Toast message={"EPG is outdated!"} button={"Update"} action={() => setSynced(false)} />}
